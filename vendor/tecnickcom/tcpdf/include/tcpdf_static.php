...
class TCPDF_STATIC {
    ...
    protected static function getCurloptDefault() {
        $opts = [];
        if (defined('CURLOPT_CONNECTTIMEOUT')) { $opts[CURLOPT_CONNECTTIMEOUT] = 5; }
        if (defined('CURLOPT_MAXREDIRS')) { $opts[CURLOPT_MAXREDIRS] = 5; }
        if (defined('CURLOPT_PROTOCOLS')) {
            $proto = 0;
            if (defined('CURLPROTO_HTTPS')) { $proto |= CURLPROTO_HTTPS; }
            if (defined('CURLPROTO_HTTP'))  { $proto |= CURLPROTO_HTTP; }
            if (defined('CURLPROTO_FTP'))   { $proto |= CURLPROTO_FTP; }
            if (defined('CURLPROTO_FTPS'))  { $proto |= CURLPROTO_FTPS; }
            if ($proto !== 0) { $opts[CURLOPT_PROTOCOLS] = $proto; }
        }
        if (defined('CURLOPT_SSL_VERIFYHOST')) { $opts[CURLOPT_SSL_VERIFYHOST] = 2; }
        if (defined('CURLOPT_SSL_VERIFYPEER')) { $opts[CURLOPT_SSL_VERIFYPEER] = true; }
        if (defined('CURLOPT_TIMEOUT')) { $opts[CURLOPT_TIMEOUT] = 30; }
        if (defined('CURLOPT_USERAGENT')) { $opts[CURLOPT_USERAGENT] = 'tcpdf'; }
        return $opts;
    }
    ...
    protected static function getCurloptFixed() {
        $opts = [];
        if (defined('CURLOPT_FAILONERROR')) { $opts[CURLOPT_FAILONERROR] = true; }
        if (defined('CURLOPT_RETURNTRANSFER')) { $opts[CURLOPT_RETURNTRANSFER] = true; }
        return $opts;
    }
