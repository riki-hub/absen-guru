...
$header = ['No', 'Nama Guru', 'Tanggal', 'Jam 0', 'Jam 1', 'Jam 2', 'Jam 3', 'Jam 4', 'Jam 5', 'Jam 6', 'Jam 7', 'Jam 8', 'Jam 9', 'Jam 10', 'Total absen'];
$column = 'A';
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Periksa setiap kolom jam dan tambahkan ke jumlah jika ada isinya
    for ($i = 0; $i <= 10; $i++) {
        if (!empty($row['jam' . $i])) $jumlahJam++;
    }
    ...
    $sheet->setCellValue('A' . $rowNumber, $no)
        ...
        ->setCellValue('K' . $rowNumber, $row['jam7'])
        ->setCellValue('L' . $rowNumber, $row['jam8'])
        ->setCellValue('M' . $rowNumber, $row['jam9'])
        ->setCellValue('N' . $rowNumber, $row['jam10'])
        ->setCellValue('O' . $rowNumber, $jumlahJam);

    // Apply red background color to cells with empty values
...
// Apply italic and center alignment to columns D to M
$italicCenterStyle = [
    'font' => [
        'italic' => true,
    ],
...
$sheet->getStyle('D2:O' . ($rowNumber - 1))->applyFromArray($italicCenterStyle);
